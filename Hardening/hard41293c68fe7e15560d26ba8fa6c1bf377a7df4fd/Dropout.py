...
parser.add_argument('--dropout_rate', default=0, help='calib_iter')
...
device = 'cuda:0'
min_level, max_level, start_level = 1, 30, 1 # index range of path difficulties to train and evaluate on, inclusive
output_dir = f'/leonardo_work/IscrC_HardNet/dropout/{args.dropout_rate}/'
dropout_rate = float(args.dropout_rate)
continue_training = False
continue_model_at = None
continue_config_at = None
# continue_training = True
# continue_model_at = f'/leonardo_work/IscrC_HardNet/dropout/{dropout_rate}/modeling/model_checkpoint.zip' # specify model path to continue training from
# continue_config_at = f'/leonardo_work/IscrC_HardNet/dropout/{dropout_rate}/modeling/configuration_model_checkpoint.json' # specify configuration path to continue training from
dropout_scale = False 
# learning algorithm
total_timesteps = 1_000_000_000 # maximum number of timesteps to train on
	# SB3 default is 1e6, Microsoft uses 5e5
buffer_size = 200_000 # number of recent steps (observation, action, reward) tuples to store in memory to train on -- takes up memory
